...
import PrepIt from "../components/PrepIt";
import DebouncedPressable from "../../../common/components/DebouncePressable";
import PrepFavorite from "../components/PrepFavorite";
import PrepInfoBox from "../components/PrepInfoBox";
import PrepVideo from "../components/PrepVideo";

type PrepScreenProps = NativeStackScreenProps<MakeStackParamList, "PrepDetail">;

export default function PrepScreen({ route, navigation }: PrepScreenProps) {
    ...
    <SafeAreaView style={tw`flex-1 bg-white`}>
      ...
      >
        <View style={tw`flex-row items-center px-4 py-3`}>
          <Pressable onPress={() => navigation.goBack()} style={tw`mr-4`}>
            <Ionicons name="arrow-back" size={26} color="#245235" />
          </Pressable>
          <Animated.Text
            numberOfLines={1}
            style={[
              tw`font-bold text-[#245235] flex-1`,
              { fontSize: 16, opacity: headerTitleOpacity },
            ]}
          >
      ...
      <Animated.ScrollView
        ...
        contentContainerStyle={[
          tw`pb-40`,
          { paddingTop: (StatusBar.currentHeight || 0) + 90 }, // fixes hidden title
        ]}
      ...
      >
        ...
        <Animated.View style={{ opacity: largeTitleOpacity }}>
          ...
          {/* Save Meal */}
          <PrepFavorite saved={saved} setSaved={setSaved} />
        </Animated.View>

        {/* Info Section */}
        <PrepInfoBox recipe={recipe} />
