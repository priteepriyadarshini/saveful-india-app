...
interface PrepFavoriteProps {
  saved: boolean;
  setSaved: React.Dispatch<React.SetStateAction<boolean>>;
}

export default function PrepFavorite({ saved, setSaved }: PrepFavoriteProps) {
  return (
    <DebouncedPressable onPress={() => setSaved(!saved)}>
      <View style={tw`items-center pb-2`}>
        <View
          style={tw.style(
            "w-[220px] flex-row items-center justify-center rounded-full border px-3 py-[3px]",
            saved ? "border-eggplant bg-eggplant" : "border-black"
          )}
        >
          <Feather
            name="star"
            size={16}
            color={saved ? tw.color("lemon") : tw.color("black")}
          />
          <Text
            style={tw.style(
              subheadLargeUppercase,
              "ml-2",
              saved ? "text-lemon" : "text-black"
            )}
          >
