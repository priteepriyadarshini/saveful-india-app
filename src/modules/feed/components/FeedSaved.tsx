...
import { cardDrop } from '../../../theme/shadow';
import { h6TextStyle, subheadLargeUppercase, subheadSmallUppercase } from '../../../theme/typography';
import useEnvironment from '../../../modules/environment/hooks/useEnvironment';

// Dummy stats type
interface Stats {
  food_savings_user: number;
  food_savings_all_users: number;
}
...
export default function FeedSaved() {
  ...
  useEffect(() => {
    ...
    //   const response = await fetch(GET_QUANTITY_SAVED_BY_USER);
    //   const data = await response.json();
    //   setStats({
    //     food_savings_user: data.food_savings_user,
    //     food_savings_all_users: data.food_savings_all_users,
    //   });
    // };

    // Dummy data
    setStats({
      food_savings_user: 12.34,
      food_savings_all_users: 567.89,
    });
  }, []);
  ...
  return (
    <View style={tw`relative`}>
      <View
        style={[
          tw.style(`mx-5 overflow-hidden rounded-2lg border border-radish bg-black shadow-sm`),
          cardDrop,
        ]}
      >
        <View style={tw`border-b border-radish py-3`}>
          <Text style={tw.style(subheadLargeUppercase, 'text-center text-creme')}>
            Stats and potential savings
          </Text>
        </View>
        ...
        <View style={tw`flex-row items-center justify-between gap-3 px-2 py-3`}>
          ...
          <View style={tw`grow items-center gap-1`}>
            <Text
              style={tw.style(subheadSmallUppercase, 'mx-auto text-center text-white')}
              maxFontSizeMultiplier={1}
            >
              You’ve saved
            </Text>
            <Text style={tw.style(h6TextStyle, 'text-radish')} maxFontSizeMultiplier={1}>
              {Number(stats.food_savings_user).toFixed(2)}
            </Text>
            <Text
              style={tw.style(subheadSmallUppercase, 'text-center text-white')}
          ...
          <View style={tw`grow items-center gap-1 overflow-hidden`}>
            ...
            <Text style={tw.style(h6TextStyle, 'text-mint')} maxFontSizeMultiplier={1}>
              {Number(stats.food_savings_all_users).toFixed(2)}
            </Text>
            <Text
              style={tw.style(subheadSmallUppercase, 'text-center text-white')}
